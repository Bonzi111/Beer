...
export class AddToCartComponent implements OnInit {
  ...
  onClick(object) {
    ...
    for(var i=0; i<this.like_values.length; i++){
      if(this.like_values[i].email==this.email && this.like_values[i].id==object.company_id){
        this.index=i;
}
}
    this.db_key=this.like_keys[this.index];
   
    this.signup_obj.deletePost(this.db_key).subscribe((res) => {
      this.signup_obj.get_like().subscribe((data) => {
        this.like_db = data;
        ...
        const value = this.like_values.filter(a => a.email == this.email);
        console.log(value);
        this.array = value.map((ele) => {
          return ele.id;
});
...
// }
//         this.unique_array=removeDups(this.array);
        console.log(this.array);
});
